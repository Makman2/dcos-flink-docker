...
RUN tar -xzf /tmp/flink.tar.gz && rm /tmp/flink.tar.gz
...
#ln -s $(pwd)/lib/flink-dist_2.10-1.2-SNAPSHOT.jar $MESOS_SANDBOX/flink.jar \
...
CMD $JAVA_HOME/bin/java -cp "lib/*" -Dlog.file=jobmaster.log -Dlog4j.configuration=file:log4j.properties org.apache.flink.mesos.runtime.clusterframework.MesosApplicationMasterRunner --configDir .
